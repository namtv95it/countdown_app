...
with open("lib/widgets/effect_background.dart", "r", encoding="utf-8") as f:
    content = f.read()

# 1. Update _getParticleCount
old_count = """  int _getParticleCount() {
...
new_count = """  int _getParticleCount() {
    switch (_currentEffect) {
      case 'bubbles': return 30;
      case 'hearts': return 25;
      case 'snow': return 50;
...
# 3. Update AnimatedBuilder
old_anim = """          if (_currentEffect == 'bubbles') {
            p.x += sin(p.y * 0.03) * 0.4;
          } else if (_currentEffect == 'hearts') {"""
new_anim = """          if (_currentEffect == 'bubbles') {
            p.x += sin(p.y * 0.03) * 0.4;
...
old_custom_paint = """        return CustomPaint(
          painter: EffectPainter(particles: _particles, effectType: _currentEffect),
          size: Size.infinite,
        );"""
new_custom_paint = """        return CustomPaint(
          painter: EffectPainter(particles: _particles, effectType: _currentEffect, time: _controller.value),
          size: Size.infinite,
        );"""
content = content.replace(old_custom_paint, new_custom_paint)

# 4. Update EffectPainter class definition
old_painter = """class EffectPainter extends CustomPainter {
...
new_painter = """class EffectPainter extends CustomPainter {
  final List<Particle> particles;
  final String effectType;
  final double time;
...
# 5. Add new drawing methods to the end of EffectPainter
old_end = """  @override
  bool shouldRepaint(covariant CustomPainter oldDelegate) => true;
}"""
new_end = """  void _drawLeaf(Canvas canvas, Particle p, Paint paint) {
    canvas.save();
...
with open("lib/widgets/effect_background.dart", "w", encoding="utf-8") as f:
    f.write(content)
